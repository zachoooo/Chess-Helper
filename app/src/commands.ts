...
export const commands: Record<string, () => void> = {
  ...
  resign: () => {
    const resignButton = <Nullable<HTMLButtonElement>>(
      document.querySelector(".resign-button-component")
    );
    resignButton && resignButton.click();
  },
  draw: () => {
    const drawButton = <Nullable<HTMLButtonElement>>(
      document.querySelector(".draw-button-component")
    );
    drawButton && drawButton.click();
  },
  focus: () => {
    const focusButton = <Nullable<HTMLButtonElement>>(
      document.querySelector("#board-controls-focus")
    );
    focusButton && focusButton.click();
  },
};
